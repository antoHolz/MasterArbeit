...
def PI(x):
    mu, sigma = opt.gp.predict_noiseless(x)
    _, fmax= opt.get_maximum_S()
    u=(mu-fmax-0.01)/sigma
...
if __name__ == "__main__":
    ...
    parser.add_argument('--duration_sec',       default=480,         type=int,           help='Duration of the simulation in seconds (default: 5)', metavar='')
    ...
    ############################################################
    dataPI=pd.DataFrame(index=['max_PI', 'global_max', 'id_gm', 'simple_max', 'id_sm'])
    ...
    for i in range(0, int(ARGS.duration_sec*env.SIM_FREQ), AGGR_PHY_STEPS):
        ...
        if i%CTRL_EVERY_N_STEPS == 0:
            ...
            for j in range(ARGS.num_drones):
                ...
                # t1 = time.time()
                # print("control calculation: "+ str(t1-t0))
            ...
            if(((i+CTRL_EVERY_N_STEPS)%ROUND_STEPS) == TRAJ_STEPS):
                ...
                # t3 = time.time()
                # print("performance calculation: "+ str(t3-t2))
            ...
            if(i==0 or ((i%ROUND_STEPS) == TRAJ_STEPS)):
                if(learn):
                    ...
                    if(learn_counter==1):
                        ...
                        #PIstuff
                        p_axis=np.linspace(0,2,6)
                        i_axis=np.linspace(0,1,4)
                        grid=np.array(np.meshgrid(p_axis,p_axis,p_axis,i_axis,i_axis,i_axis)).T.reshape(-1,6)
                    #### Add new point to the GP model  ###############
                    elif(learn_counter>1):
                        
                        opt.add_new_data_point(n_candidate,  normalized_cost) 
                    # t5 = time.time()
                    # print("GP actualization: "+ str(t5-t4))
                    #### GP-Plot ######################
                ...
                if(learn_counter>=1):
                    ...
                    if(np.abs(normalized_cost-mu_t)>(np.sqrt(rho_t)*np.sqrt(var_t)+omega_t)):
                        ...
                        learn_at=np.concatenate((learn_at, np.expand_dims(np.array(round_nr+1),0)))

                    if(learn_counter==1):
                        Rho=np.expand_dims(rho_t,0)
                        E_t=np.expand_dims((np.sqrt(rho_t)*np.sqrt(var_t)).squeeze(),0)
                    else:
                        Rho=np.concatenate((Rho, np.expand_dims(rho_t,0)))
                        E_t=np.concatenate((E_t, np.expand_dims((np.sqrt(rho_t)*np.sqrt(var_t)).squeeze(),0)))

                # t7 = time.time()
                # print("learning trigger: " + str(t7-t6))
                ############################################################
                ...
                if(learn):
                    ...
                    if(learn_counter==0):
                    ...
                    elif((learn_counter)<max_learn_rounds and early_stop<3): #and (cost<=-80)):              
                        ...
                        t1=time.time()
                        result_pi=PI(grid)
                        max_PI=np.max(result_pi)
                        print("PI max:  "+str(max_PI))
                        if(max_PI<1e-5):
                            early_stop+=1
                        t2=time.time()
                        print("time:    "+str(t2-t1))
                        t3=time.time()
                        idGmax, gmax=opt.get_maximum_S()
                        t4=time.time()
                        print("time global reward: "+str(t4-t3))
                        idGmax=np.multiply(idGmax.squeeze(),theta_norm)
                        gmax=gmax*cost_norm
                        print("global max: "+str(gmax)+" at: "+str(idGmax))
                        t5=time.time()
                        idsmax, smax= opt.get_maximum()
                        t6=time.time()
                        print("time observed reward: "+str(t6-t5))
                        idsmax=np.multiply(idsmax.squeeze(),theta_norm)
                        smax=smax.item()*cost_norm
                        print("simple max: "+str(smax)+" at:"+str(idsmax))
                        
                        dataPI=dataPI.append({'max_PI':max_PI, 'global_max':gmax,
                             'id_gm':idGmax, 'simple_max':smax, 'id_sm':idsmax}, ignore_index=True)
                        ...
                        early_stop=0
                    # t9 = time.time()
                    # print("BO new candidate: "+ str(t9-t8))
                learn_counter+=1
                # t11=t10
                # t10=time.time()
                # print("ROUND: "+ str(t10-t11)+"s")
    ...
    learn_rounds=pd.DataFrame(learn_at)

    others=pd.DataFrame(np.vstack((Rho, E_t)))

    AQUISITION_F="st"
    PID_START="0.4_0.05"
    NR_ROUNDS=str(int(ARGS.duration_sec/PERIOD))
    FILENAME="fly8_learn"+"_start_PI_"+PID_START+"_rounds_"+NR_ROUNDS
    logger.save_as_csv_w_performance(FILENAME, candidates, learn_rounds, others)

    dataPI.to_csv('C:/Users/Usuario/Documents/Masterarbeit/code/gym-pybullet-drones-1.0.0/files/csvs/dataPI.csv')
