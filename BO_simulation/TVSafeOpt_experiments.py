...
if __name__ == "__main__":
    ...
    parser.add_argument('--duration_sec',       default=60*12+1*24, type=int,           help='Duration of the simulation in seconds (default: 5)', metavar='')
    ...
    parser.add_argument('--max_learn_rounds',   default=20,         type=int,           help='Maximum learning rounds for SafeOpt algorithm', metavar='')
    ...
    for i in range(NUM_WP):
        TARGET_POS[i+NUM_R, :] = R * np.cos((i / NUM_WP) * (2 * np.pi) + np.pi / 2) + INIT_XYZS[0, 0], R * np.sin(
            2*((i / NUM_WP) * (2 * np.pi) + np.pi / 2))/2 + INIT_XYZS[0, 1],  INIT_XYZS[0, 2]
    for i in range(NUM_R):
        TARGET_POS[i, :] = INIT_XYZS[0,:]
    ...
    for g in range(10):
        ...
        for i in range(0, int((ARGS.duration_sec-24)*env.SIM_FREQ), AGGR_PHY_STEPS):
            ...
            if i%CTRL_EVERY_N_STEPS == 0:
                ...
                for j in range(ARGS.num_drones):
                    ...
                    if(ARGS.experiment=="PWM"):
                        ### Update PWM ############################
                        if(i/ROUND_STEPS==29):
                            ctrl[j].setPWM2RPM_scale_hw(0.26)
                            hover(24,j)
                        elif(i/ROUND_STEPS==59):
                            ctrl[j].setPWM2RPM_scale_hw(0.2685)
                            hover(24,j)
                    elif(ARGS.experiment=="ATT"):
                        # #### detuning AC ############################
                        if(i/ROUND_STEPS==30): 
                            PID_coeff=np.array([[0.4, 0.4, 1.25],[.05, .05, .05],[.2, .2, .5], #PID xyz
                            [35000., 35000., 30000.],[.0, .0, 500.],[20000., 20000., 12000.]]) #PID rpy
                ...
                if not ARGS.baseline:
                    if((i%ROUND_STEPS) == 0):
                        ...
                        if(learn):
                            ...
                            if(learn_counter==1):
                                ...
                                #### Define Kernel
                                lengthscale=[0.15/theta_norm[0], 0.15/theta_norm[1],  
                    ...
                    if((i>=ROUND_STEPS) & ((i%ROUND_STEPS) == 0)):
                        ...
                        for i in range(2):
                            PID_coeff[i]=np.reshape(cand[3*i:3*i+3].squeeze(),PID_coeff[i].shape)
                        ctrl[j].setPIDCoefficients(*PID_coeff)  
        ...
        NR_ROUNDS=str(int(ARGS.duration_sec/PERIOD)-2)
        if ARGS.baseline:
            ALGORITHM="baseline"
        elif(ARGS.ET):
        ...
        FILENAME="Experiments_v3/"+ARGS.experiment+"/"+ALGORITHM+"/"+ALGORITHM+"_rounds_"+NR_ROUNDS+"_AC70_35_70"
