...
class Logger(object):
    ...
    def save_as_csv(self,
        ...
        """
        csv_dir = os.environ.get('HOME')+"/Desktop/save-flight-"+comment+"-"+datetime.now().strftime("%m.%d.%Y_%H.%M")
    ...
    ################################################################################
    def save_as_csv_w_performance(self,
                    comment: str="",
                    candidates: object=pd.DataFrame(),
                    learn_rounds: object=pd.DataFrame(),
                    others: object=pd.DataFrame()
                    ):
        """Save the logs---on your Desktop---as comma separated values.
        ...
        candidates.to_csv(csv_dir+'/candidate_performance.csv')
        if not learn_rounds.empty:
            learn_rounds.to_csv(csv_dir+'/learn_rounds.csv')
        if not others.empty:
            others.to_csv(csv_dir+'/others.csv')
