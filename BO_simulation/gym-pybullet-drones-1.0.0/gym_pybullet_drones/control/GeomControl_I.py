...
class GeomControl(BaseControl):
    ...
    def __init__(self, 
        ...
        if (drone_type == 'cf2'):
            ...
            self.k_I=np.array([0.05, 0.05, 0.05])
    ...
    def compute_att_control(self,
        ...
        self.int_pos_e = self.int_pos_e + self.dt * pos_e
        if np.abs(den) > 1e-7:
            thrust = (-self.k_r[2] * pos_e[2] - self.k_v[2] * vel_e[2] - self.k_I[2]*self.int_pos_e[2]+ self.mass * self.gravity +
...
################################################################################
    ...
    def setCtrlCoefficients(self,
        ...
        if not all(hasattr(self, attr) for attr in ATTR_LIST):
            ...
            self.k_I[0:2] = self.k_I[0:2] if K_Ix is None else K_Ix
            self.k_I[2] = self.k_I[2] if K_Iz is None else K_Iz
