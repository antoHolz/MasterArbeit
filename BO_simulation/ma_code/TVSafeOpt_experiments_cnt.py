...
def hover(sec, j):
    for g in range(0,sec*env.SIM_FREQ, AGGR_PHY_STEPS):
        #### Step the simulation ###################################
        obs, reward, done, info = env.step(action, int(i/env.SIM_FREQ))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--duration_sec',       default=60*12, type=int,           help='Duration of the simulation in seconds (default: 5)', metavar='')
    ...
    parser.add_argument('--max_learn_rounds',   default=10,         type=int,           help='Maximum learning rounds for SafeOpt algorithm', metavar='')
    ...
    for g in range(ARGS.repetitions):
            ...
            if i%CTRL_EVERY_N_STEPS == 0:
                ...
                for j in range(ARGS.num_drones):
                    ...
                    if(ARGS.experiment=="PWM_ns"):
                        ### Update PWM ############################
                        if(i/ROUND_STEPS==2):
                            ctrl[j].setPWM2RPM_scale_hw([[0.2685,0.250,.250,.250]])
                            hover(24,j)
                    ...
                    if((i%ROUND_STEPS) == 0):
                        ...
                        if(learn):
                            ...
                            if(learn_counter==1):
                                ...
                                #### Define Kernel
                                lengthscale=[0.10/theta_norm[0], 0.15/theta_norm[1],  
        ...
        FILENAME="Experiments_v3/"+ARGS.experiment+"_varM/"+ALGORITHM+"/"+ALGORITHM+"_rounds_"+NR_ROUNDS
