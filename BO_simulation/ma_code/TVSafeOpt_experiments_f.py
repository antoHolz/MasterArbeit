...
if __name__ == "__main__":
    ...
    parser.add_argument('--duration_sec',       default=7*40,      type=int,           help='Duration of the simulation in seconds (default: 5)', metavar='')
    ...
    parser.add_argument('--max_learn_rounds',   default=19,         type=int,           help='Maximum learning rounds for SafeOpt algorithm', metavar='')
    ...
    parser.add_argument('--repetitions',        default=5,         type=int,           help='Number of repetitions of the experiment', metavar='')
    ...
    for g in range(ARGS.repetitions):
        ...
        if ARGS.drone in [DroneModel.CF2X, DroneModel.CF2P]:
            ctrl = [GeomControl(drone_model=ARGS.drone, dt=CTRL_EVERY_N_STEPS*env.TIMESTEP) for i in range(ARGS.num_drones)]
        ...
        for i in range(ARGS.num_drones):
            ctrl[i].setCtrlCoefficients(*PID_coeff) 
        #### Safety constraints ####################################
        delta=0.01
        #safe_start=np.array([0.4, 1.25,.05, .05])
        safe_start=np.array([.8, 8e-2]) #np.array([0.4, 1.25,.05, .05])#
        ...
        for i in range(0, int((ARGS.duration_sec)*env.SIM_FREQ), AGGR_PHY_STEPS):
            ...
            if i%CTRL_EVERY_N_STEPS == 0:
                ...
                for j in range(ARGS.num_drones):
                    ...
                    if(ARGS.experiment=="flip_mass"):
                        ...
                        if(i/ROUND_STEPS==0):
                            TARGET_POS, TARGET_VEL, TARGET_ACC, TARGET_QUAT, TARGET_ANG_VEL=utils.get_flip_trajectory(0.9, PERIOD_8+RETURN_TIME-0.9, ARGS.control_freq_hz, H)
                            #mass setting happens in BaseAviary
            ...
            #print(action[str(j)])
                ...
                if(((i+CTRL_EVERY_N_STEPS)%ROUND_STEPS) == 0):
                    ...
                    cost=-np.sum(np.sqrt(np.sqrt(distances)))-100*np.sum(err_end)
                ...
                if not ARGS.baseline:
                    if((i%ROUND_STEPS) == 0):
                        ...
                        if(learn):
                            ...
                            if(learn_counter==1):
                                ...
                                #### Define Kernel
                                lengthscale=[0.1/theta_norm[0], 0.01/theta_norm[1],  
                    ...
                    if((i%ROUND_STEPS) == 0):
                        ...
                        ctrl[j].setCtrlCoefficients(K_vz=candidate[0], K_R=candidate[1])  
