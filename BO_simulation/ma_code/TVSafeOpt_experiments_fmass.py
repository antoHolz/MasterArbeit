...
The control is given by the PID implementation in `GeomControl`.
...
def hover(sec, j):
    for g in range(0,sec*env.SIM_FREQ, AGGR_PHY_STEPS):
        #### Step the simulation ###################################s
        obs, reward, done, info = env.step(action,i)
        #### Compute control at the desired frequency ##############
        if g%CTRL_EVERY_N_STEPS == 0:

            action[str(j)] = ctrl[j].compute_pos_control(cur_pos=obs[str(j)]["state"][0:3],
                                                                cur_quat=obs[str(j)]["state"][3:7],
                                                                cur_vel=obs[str(j)]["state"][10:13],
                                                                cur_ang_vel=obs[str(j)]["state"][13:16],
                                                                target_pos=[0,0,1]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--physics',            default="pyb_mc",      type=Physics,       help='Physics updates (default: PYB)', metavar='', choices=Physics)
    ...
    parser.add_argument('--duration_sec',       default=276,      type=int,           help='Duration of the simulation in seconds (default: 5)', metavar='')
    ...
    parser.add_argument('--experiment',         default="flip_mass",      type=str,           help='Experiment kind(ATT,PWM,GE,none)', metavar='')
    parser.add_argument('--repetitions',        default=10,         type=int,           help='Number of repetitions of the experiment', metavar='')
    ...
    H_STEP = .05
    R = .3
    INIT_XYZS = np.array([[0, 0, H+j*H_STEP] for j in range(ARGS.num_drones)])#np.array([[R*np.cos((i/6)*2*np.pi+np.pi/2), R*np.sin(((i/6)*2*np.pi+np.pi/2)*2)/2, H+i*H_STEP] for i in range(ARGS.num_drones)])
    INIT_RPYS = np.array([[0, 0,  j * (np.pi/2)/ARGS.num_drones] for j in range(ARGS.num_drones)])
    ...
    TARGET_POS = utils.get_trajectory_xy(NUM_WP,NUM_R, [0,0,1], .3)#default trajectory

        # TARGET_POS = np.zeros((NUM_WP+NUM_R, 3))
        # for i in range(NUM_WP+NUM_R):
        #     TARGET_POS[i, 2]=1
    ...
    wp_counters = np.array([int((j * (NUM_WP+NUM_R) / 6) % (NUM_WP+NUM_R)) for j in range(ARGS.num_drones)])
    ...
    for g in range(ARGS.repetitions):
        ...
        if ARGS.drone in [DroneModel.CF2X, DroneModel.CF2P]:
            ctrl = [GeomControl(drone_model=ARGS.drone, dt=CTRL_EVERY_N_STEPS*env.TIMESTEP) for j in range(ARGS.num_drones)]
        elif ARGS.drone in [DroneModel.HB]:
            ctrl = [GeomControl(drone_model=ARGS.drone, dt=CTRL_EVERY_N_STEPS*env.TIMESTEP) for j in range(ARGS.num_drones)]

        #### Run the simulation ####################################
        action = {str(d): np.array([0,0,0,0]) for d in range(ARGS.num_drones)}
        START = time.time()
        
        ############################################################
        #### start PID parameter ###################################
        PID_coeff=np.array([.5,1.25,.2,.8,8e-2,2e-3]) #np.array([4.5,4.5,.3,.3,.08,.002]) # 

        for j in range(ARGS.num_drones):
            ctrl[j].setCtrlCoefficients(*PID_coeff) 
        #### Safety constraints ####################################
        delta=0.01
        #safe_start=np.array([0.4, 1.25,.05, .05])
        safe_start=np.array([1.25, .8, 8e-2]) #np.array([0.4, 1.25,.05, .05])#
        ...
        for i in range(0, int((ARGS.duration_sec)*env.SIM_FREQ), AGGR_PHY_STEPS):
            ...
            if i%CTRL_EVERY_N_STEPS == 0:
                ...
                for j in range(ARGS.num_drones):
                    ...
                    if(ARGS.experiment == "traj_3"):
                        if(i/ROUND_STEPS==0):
                            TARGET_POS=utils.get_trajectory_xy(NUM_WP,NUM_R, INIT_XYZS[0,:], R)
                        elif(i/ROUND_STEPS==20):
                            TARGET_POS=utils.get_trajectory_xz(NUM_WP, NUM_R,INIT_XYZS[0,:], R)
                    
                    if(ARGS.experiment=="flip_PWM"):
                        ### Update PWM ############################
                        if(i/ROUND_STEPS==0):
                        ...
                        if(i/ROUND_STEPS==20):
                            ctrl[j].setRPMfactor(0.9)
                    
                    if(ARGS.experiment=="flip_mass"):
                        ### Update in BA ############################
                        if(i/ROUND_STEPS==0):
                            TARGET_POS, TARGET_VEL, TARGET_ACC, TARGET_QUAT, TARGET_ANG_VEL=utils.get_flip_trajectory(0.9, PERIOD_8+RETURN_TIME-0.9, ARGS.control_freq_hz, H)
                            #mass setting happens in BaseAviary_plus
                    
                    if(ARGS.experiment=="GE"):
                        if(i/ROUND_STEPS==0):
                            TARGET_POS, TARGET_VEL, TARGET_ACC, TARGET_QUAT, TARGET_ANG_VEL=utils.get_flip_trajectory(0.9, PERIOD_8+RETURN_TIME-0.9, ARGS.control_freq_hz, H)
                            flip=True
                        elif(i/ROUND_STEPS==20):
                            ctrl[j].setRPMfactor(0.8)
                        ### Update soll trajectory ############################
                        if(i/ROUND_STEPS==20):
                            TARGET_POS=utils.get_trajectory_xy(NUM_WP,NUM_R, [0,0,.05], R)
                            hover(24,j)
                    ...
                    if(ARGS.experiment=="xy-xz-flip"):
                        ...
                        if(i/ROUND_STEPS==100):
                            ...
                            TARGET_POS, TARGET_VEL, TARGET_ACC, TARGET_QUAT, TARGET_ANG_VEL=utils.get_flip_trajectory(0.9, PERIOD_8+RETURN_TIME-0.9, ARGS.control_freq_hz, H)
                            #TARGET_POS, TARGET_VEL, TARGET_ACC, TARGET_QUAT, TARGET_ANG_VEL=utils.flip(0.9, RETURN_TIME, ARGS.control_freq_hz, H)
                            flip=True
                    if(np.all(TARGET_QUAT[wp_counters[j], 0:4]==0)):# and np.all(TARGET_ACC[wp_counters[j], 0:4]==0) and np.all(TARGET_ANG_VEL[wp_counters[j], 0:4]==0)):
                        action[str(j)]= ctrl[j].compute_pos_control(cur_pos=obs[str(j)]["state"][0:3],
                                                                    cur_quat=obs[str(j)]["state"][3:7],
                                                                    cur_vel=obs[str(j)]["state"][10:13],
                                                                    cur_ang_vel=obs[str(j)]["state"][13:16],
                        ...
                        #print('PC')
                        #print(TARGET_POS[wp_counters[j], 0:3])
                    else:
                        action[str(j)]= ctrl[j].compute_att_control(cur_pos=obs[str(j)]["state"][0:3],
                                                                    cur_quat=obs[str(j)]["state"][3:7],
                                                                    cur_vel=obs[str(j)]["state"][10:13],
                                                                    cur_ang_vel=obs[str(j)]["state"][13:16],
                                                                    target_pos=TARGET_POS[wp_counters[j], 0:3],
                                                                    target_vel=TARGET_VEL[wp_counters[j], 0:3],
                                                                    target_acc=TARGET_ACC[wp_counters[j], 0:3],
                                                                    target_quat=TARGET_QUAT[wp_counters[j], 0:4],
                                                                    target_ang_vel=TARGET_ANG_VEL[wp_counters[j], 0:3]
                                                                    )
                        #print('AC')
            #print(action[str(j)])
                ...
                if(((i+CTRL_EVERY_N_STEPS)%ROUND_STEPS) == 0):
                    ...
                    #U=np.squeeze(U)
                    err_end=np.abs(X_ist[-1]-TARGET_POS[-1])**0.25
                    err_strt=np.abs(X_ist[0]-INIT_XYZS)**0.25
                    cost=-100*np.sum(err_end)-100*np.sum(err_strt) #-np.sum(np.sqrt(np.sqrt(distances))) #
                ...
                if not ARGS.baseline:
                    if((i%ROUND_STEPS) == 0):
                        ...
                        if(learn):
                            ...
                            if(learn_counter==1):
                                ...
                                bounds = [(1e-2, 1e1),(1e-2, 1e1), (1e-2, 1e1),#Px
                                        #(0, 1e1), (0, 1e1),#Py
                                        # (0, 1e1), (0, 1e1), #Pphi
                                        ] 
                                ...
                                #### Define Kernel
                                lengthscale=[.8/theta_norm[0], .10/theta_norm[1], 0.010/theta_norm[2],  
                                            #0.05/theta_norm[2], 0.05/theta_norm[3],  
                                            #4e-2/theta_norm[4], 1e-3/theta_norm[5],
                                            ]
                                EPS=0.3
                        ...
                        #### Obtain next query point ########################### 
                            if(learn_counter==0):
                                ...
                                (learn_counter>=10 and (learn_counter+1)%5==0)):
                    ...
                    if((i%ROUND_STEPS) == 0):
                        ...
                        ctrl[j].setCtrlCoefficients(K_rz=candidate[0], K_vz=candidate[1], K_R=candidate[2])  
                        # print(ctrl[j].getCtrlCoefficients())
        ...
        FILENAME="Experiments_v3/"+ARGS.experiment+"/"+ALGORITHM+"/"+ALGORITHM+"_rounds_"+NR_ROUNDS
