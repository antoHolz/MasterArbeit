...
if __name__ == "__main__":
    ...
    for i in range(NUM_WP):
        TARGET_POS[i, :] = R * np.cos((i / NUM_WP) * (2 * np.pi) + np.pi / 2) + INIT_XYZS[0, 0], R * np.sin(
            2*((i / NUM_WP) * (2 * np.pi) + np.pi / 2))/2 + INIT_XYZS[0, 1], 0
    wp_counters = np.array([int((i * NUM_WP / 6) % NUM_WP) for i in range(ARGS.num_drones)])
    ...
    #PID_coeff=np.array([[0.01, 0.01, 0.01],[.05, .05, .05],[.2, .2, .5],[70000., 70000., 60000.],[.0, .0, 500.],[20000., 20000., 12000.]])
    ...
    ##  Uncomment next line if csv save presents TypeError: unsupported operand type(s) for +: 'NoneType' and 'str'
    os.environ["HOME"]='C:/Users/Usuario' 
