...
if __name__ == "__main__":
    ...
    #start PID parameter
    PID_coeff=np.array([[0.1, 0.1, 1.25],[.05, .05, .05],[.2, .2, .5], #PID xyz
    ...
    for i in range(0, int(ARGS.duration_sec*env.SIM_FREQ), AGGR_PHY_STEPS):
        ...
        if i%CTRL_EVERY_N_STEPS == 0:
            ...
            if(int((i+ARGS.simulation_freq_hz*RETURN_TIME)/ROUND_STEPS)==10):
                TARGET_POS=TARGET_POS_2
            elif(int((i+ARGS.simulation_freq_hz*RETURN_TIME)/ROUND_STEPS)==20):
                ...
                if(learn):
                    ...
                    if(learn_counter==1):
                        ...
                        def beta(learn_counter):
                            return 0.8*np.log(2*learn_counter)   #bogunovic
