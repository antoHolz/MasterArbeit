...
def get_trajectory_gerono_2(z, R, h):
    X_soll=get_trajectory_gerono(z, R)
    NUM_WP=len(X_soll[:,0])
    Z=np.zeros(NUM_WP)
...
if __name__ == "__main__":
    ...
    for rounds in range(total_rounds):
        ...
        if(learn):
            ...
            if(learn_counter==1):
                #### Measurement noise
                noise_std = 0.016*np.abs(n_cost.squeeze())
        ...
        if(exp_type=="xyGBE"):
            ...
            for k in range(len(chg_at)):
                ...
                if rounds == (chg_at[k]) and k==1:    
                    ...
                    X_soll = get_trajectory_gerono_xz(Z[0], R)   
        elif(exp_type=="xy-xz-2"):
            for k in range(len(chg_at)):
                if rounds == (chg_at[k]) and k==0:    
                    print("trajectory change! New trajectory: Gerono_xy lemniscate")
                    X_soll = get_trajectory_gerono(Z[0], R)   
                if rounds == (chg_at[k]) and k==1:    
                    print("trajectory change! New trajectory: Gerono_xz_2 lemniscate")
                    X_soll = get_trajectory_gerono_2(Z[0], R, .5)   
        elif(exp_type=="xz-xy"):
            for k in range(len(chg_at)):
                if rounds == (chg_at[k]) and k==1:    
                    print("trajectory change! New trajectory: Gerono_xy lemniscate")
